...
CREATE TABLE customer (
    ...
    CONSTRAINT CHK_Username CHECK (USERNAME != 'admin' OR IS_ADMIN = 1)
);
GO
...
(1, 'admin', 'admin', '0901234567', N'Quản', N'Trị', N'123 Nguyễn Huệ, Quận 1, TP.HCM', 1),
...
INSERT INTO cart (CUSTOMER_ID, TOTAL_QUANTITY, TOTAL_PRICE) 
SELECT CUSTOMER_ID, 0, 0 FROM customer;
GO
...
-- Trigger: Update cart totals when CART_ITEM is inserted
...
BEGIN
    UPDATE c
    SET TOTAL_QUANTITY = (SELECT SUM(QUANTITY) FROM CART_ITEM WHERE CART_ID = c.CART_ID),
        TOTAL_PRICE = (SELECT SUM(SUBTOTAL) FROM CART_ITEM WHERE CART_ID = c.CART_ID)
    FROM cart c
    INNER JOIN inserted i ON c.CART_ID = i.CART_ID;
END;
GO

-- Trigger: Update cart totals when CART_ITEM is updated
...
BEGIN
    UPDATE c
    SET TOTAL_QUANTITY = (SELECT ISNULL(SUM(QUANTITY), 0) FROM CART_ITEM WHERE CART_ID = c.CART_ID),
        TOTAL_PRICE = (SELECT ISNULL(SUM(SUBTOTAL), 0) FROM CART_ITEM WHERE CART_ID = c.CART_ID)
    FROM cart c
    INNER JOIN inserted i ON c.CART_ID = i.CART_ID;
END;
GO

-- Trigger: Update cart totals when CART_ITEM is deleted
CREATE TRIGGER trg_CART_ITEM_delete
ON CART_ITEM
AFTER DELETE
AS
BEGIN
...
-- Trigger: Auto-create cart when new customer is created
...
BEGIN
    INSERT INTO cart (CUSTOMER_ID, TOTAL_QUANTITY, TOTAL_PRICE)
    SELECT CUSTOMER_ID, 0, 0 FROM inserted;
END;
GO
